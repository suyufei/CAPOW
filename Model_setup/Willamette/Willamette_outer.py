...
for res in RES:
    ...
    initial_data=pd.read_excel(os.path.join(str(path),data_filenames['@hist_res_filename']),sheetname=res.name)
...
#load cp data:
for cp in CP:
    hist_dis=pd.read_excel(os.path.join(str(path),data_filenames['@hist_cp_filename']),sheetname=cp.name)
...
Output_HP.to_excel(writer,'dams_HP_MWh')
tot_HP.to_excel(writer,'total_HP_MW')
writer.save()
