...
for res in RES:
    ...
    initial_data=pd.read_excel(data_filenames['@hist_res_filename'],sheetname=res.name)
...
#load cp data:
for cp in CP:
    hist_dis=pd.read_excel(data_filenames['@hist_cp_filename'],sheetname=cp.name)
...
T=int(sys.argv[2])  #Get the simulation horizon from input
...
writer = pd.ExcelWriter(output_filenames['@hydropower_filename'])
Output_HP.to_excel(writer,'HP_prod_MWh')
writer.save()

#control points
Output_CP=pd.DataFrame(cp_discharge_all,columns=cp_list)
writer = pd.ExcelWriter(output_filenames['@cpDischarge_filename'])
...
writer = pd.ExcelWriter(output_filenames['@summary_filename'])
